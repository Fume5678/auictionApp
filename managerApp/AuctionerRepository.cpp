...
void AuctionerRepository::remove(std::string name) {
    readBd();
    auto founded =
        std::find_if(items.begin(), items.end(), [&name](Item a) -> bool {
...
void AuctionerRepository::writeBd() {
    try {
        ...
        std::cout << "ERROR::StorageController: failed write bd" << std::endl;
...
void AuctionerRepository::readBd() {
    try {
        ...
        std::cout << "ERROR::StorageController: failed read bd" << std::endl;
        throw std::runtime_error(e.what());
    } catch (std::exception& e) {
        std::cout << "ERROR::StorageController: Failed to read Bd" << e.what()
                  << std::endl;
        throw std::runtime_error(e.what());
    }
}
